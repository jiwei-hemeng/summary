...
- flex-flow：复合属性，相当于同时设置了 flex-direction 和 flex-wrap
 ...
 ## `gap` 统一管理间距，告别 `margin` 地狱 

 以前我们写 `flex` 列表，总得用 `:not(:last-child)` 之类的选择器去掉最后一个元素的 `margin-right`。
现在直接用 `gap`： 
...
```
  ...
  不仅 `Flex` 能用，`Grid` 里 `gap` 也是一等公民。而且 `gap` 在 `flex-wrap: wrap` 换行后依然能保持合理的行间距，比挨个设置 `margin-bottom` 干净太多。 
