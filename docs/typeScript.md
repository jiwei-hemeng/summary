...
|    类型     |      核心含义      |                        关键特性与场景                        |
| :---------: | :----------------: | :----------------------------------------------------------: |
|   **any**   |    **任意类型**    | 关闭了类型检查，可以访问任何属性、赋值给任何变量。**不推荐使用**（除非遗留代码迁移）。 |
| **unknown** | **安全的任意类型** | 表示“不知道是什么类型”。**不能直接使用**，必须通过类型断言或类型守卫缩小范围后才能操作。 |
|  **void**   |    **无返回值**    | 通常用于没有 `return` 语句的函数。变量值只能是 `undefined` 或 `null`。 |
|  **never**  |  **永不存在的值**  | 用于抛出异常或无限循环的函数。它是类型系统的“底部类型”，任何类型都不能赋值给 `never`。 |

### interface 和 type 的区别

这两者功能高度重叠，主要区别在于**声明合并**和**类型范围**：

- **声明合并**：`interface` 支持声明合并（同名接口自动叠加属性），而 `type` 不可重复声明。
- **类型范围**：`type` 更灵活，可以定义联合类型、元组、原始类型别名；`interface` 主要定义对象结构。
- **扩展语法**：`interface` 用 `extends`，`type` 用 `&`（交叉类型）。
- **建议**：对外暴露的 API、开源库首选 `interface`（因为可扩展性）；内部逻辑、复杂类型（如联合类型）首选 `type`。
